...
Respond exclusively in this exact JSON schema. No preamble, no markdown fences:
{
  ...
  "taoism": "string"
    ...
    // Try primary model first, fall back to 2.0-flash on 503/overload
    ...
    try {
        ...
        for (let i = 0; i < MODELS.length; i++) {
            try {
                ...
                if (isOverload && !isLast) {
                    await sleep(2000); // brief pause before fallback
                    continue;
                }
                throw err; // not overload, or out of fallbacks
            }
        }
